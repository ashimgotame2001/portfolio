...
const DragonAnimation = () => {
    ...
    useEffect(() => {
        ...
        const run = () => {
            ...
            // Define the minimum distance between segments
            ...
            for (let i = 1; i < N; i++) {
                ...
                // Calculate the distance between current and previous segment
                const dx = ep.x - e.x;
                const dy = ep.y - e.y;
                const distance = Math.sqrt(dx * dx + dy * dy);

                // If segments are too close, push them apart
                if (distance < minDistance && distance > 0) {
                    const pushForce = (minDistance - distance) / distance;
                    e.x -= dx * pushForce * 0.5;
                    e.y -= dy * pushForce * 0.5;
                    ep.x += dx * pushForce * 0.5;
                    ep.y += dy * pushForce * 0.5;
                }
